...
if ([IO.Path]::GetFullPath($TargetHome) -eq [IO.Path]::GetFullPath($HOME) -and (Get-Command codex.exe -ErrorAction SilentlyContinue)) { & codex.exe plugin remove pi-dispatch@personal | Out-Host }
