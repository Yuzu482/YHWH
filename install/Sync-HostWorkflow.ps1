...
$json=[ordered]@{version=1;topics=$topics}|ConvertTo-Json -Depth 8
$path=Join-Path $root 'payload/pi-dispatch/workflow/catalog.json'
if($Check){if(-not(Test-Path -LiteralPath $path) -or (Get-Content -LiteralPath $path -Raw).Trim() -cne $json.Trim()){throw 'Workflow catalog is stale. Run install/Sync-HostWorkflow.ps1.'}}
