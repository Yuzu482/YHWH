...
$defaults=@{schemaVersion=1;hosts=@('codex');wslDistro='Ubuntu-24.04';installWsl=$true;allowDistroHardening=$false;installTunnel=$false;tunnelId='';tunnelRuntimeKeyFile='';tunnelClientPath=''}
...
if($config.hosts -isnot [array] -or $config.hosts.Count -eq 0){throw 'hosts must be a non-empty array.'}
foreach($hostId in $config.hosts){if($hostId -notin @('generic','codex','cherry-studio','opencode','opencode-v2','claude-code','claude-desktop','deepseek-harness','cursor','vscode-copilot','windsurf','cline','roo-code','gemini-cli','kiro','zed','continue','lm-studio')){throw "Unknown host: $hostId"}}
if(@($config.hosts|Select-Object -Unique).Count -ne $config.hosts.Count){throw 'Duplicate hosts are not allowed.'}
