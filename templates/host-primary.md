...
- The primary model is chosen in the host. Native lower workers use `openai-codex / gpt-5.6-luna` with explicit task-proportional thinking (medium by default; low/high/max per `pi-routing`); reviewers remain `anthropic / claude-sonnet-5 / max`, `access: none`, with actual review materials. The current thinking policy supersedes older fixed worker-max wording. Verify the advertised role bindings before dispatch. Do not change providers or role names to evade a failure.
