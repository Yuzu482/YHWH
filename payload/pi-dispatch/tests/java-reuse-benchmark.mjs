// Compare default tiered JIT and tier-1 JIT under identical probe isolation.
  ...
  for(let round=1;round<=2;round++){
    ...
    for(const mode of (round===1?['baseline','tuned']:['tuned','baseline'])){
      const sessions=createProbeSessions(workspace,{launch:snapshot=>{
        ...
        args.splice(at,0,'--ro-bind',path.join(stage,'java-probe.py'),'/opt/pi-kether/scripts/multilspy-probe.py','--ro-bind',path.join(stage,'java-'+mode+'.py'),'/java-probe-launch.py');
      ...
      try{
        for(const [i,request] of requests.entries()){
          ...
          if(methods[i]==='diagnostics'){assert.equal(result.diagnosticsPublished,true);assert.ok(result.result.details.data.some(d=>d.code==='16777233'));}
    ...
    assert.deepEqual(comparable.baseline,comparable.tuned,'java profiles must preserve semantic results');
  }
}
const summaries=['java'].map(language=>{
...
console.log(JSON.stringify({javaBenchmarkPassed:true,equivalentResults:true,summaries,modelCalls:0}));
