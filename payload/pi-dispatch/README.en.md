...
Cancellation acceptance: `node scripts/headless-acceptance.mjs <config> --live --cancel` first checks echo, then launches a separate invocation to cancel. Windows Job Objects clean the tree by closing the controlled job; early completion is not a cancellation pass. Upgrades start detached services, prepare backups/journals before stopping, and preserve another operation's maintenance state on conflict.

Development additions include `run-events/batch/batch-events`, `headless-acceptance.mjs`, optional Windows Job Objects and managed file upgrades. Sequential calls add bounded help/digest caches, whole-batch validation, a serial queue, phase/waiting status, per-request results and timing summaries; every call checks the version and failure stops later calls. Caches do not reuse model conversations or processes. Read-only `change-impact.mjs` combines Git, graph and knowledge-review candidates. Models and service upgrades are not invoked by default; Antigravity stays disabled with an empty path. See [operations and acceptance limits](../../docs/workflow-operations.md#english).
