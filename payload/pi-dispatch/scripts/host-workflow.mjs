...
export const workflowInstructions='YHWH provides governed Pi execution for the primary agent in this host. The primary is coordinator-only: it owns thinking, scheduling and integration, while implementation must be delegated to Pi subagents using the existing role bindings. Do not code directly or fall back to direct coding when a worker is unavailable; block instead. Before orchestration, read get_workflow(topic="primary") and pi-routing, then list_capabilities. Preserve host permissions and role bindings; fetching instructions does not attest compliance. Use discovered tool names, which may have a host prefix.';
